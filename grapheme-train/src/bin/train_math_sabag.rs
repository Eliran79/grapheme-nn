//! Math training with Sabag pooling and proper learning rates
//!
//! Tests the simplified Sabag algorithm on polish notation math
...
impl MathPair {
    fn target_polish(&self) -> String {
        // For now, train on identity: same input and output
        // This tests if model can learn to preserve structure
        self.input_polish.clone()
    }
}

fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🧮 GRAPHEME Math Training with Sabag\n");

    // Load dataset
    let file = File::open("data/generated/level_1.jsonl")?;
    let reader = BufReader::new(file);
    let mut pairs: Vec<MathPair> = Vec::new();
    ...
    for line in reader.lines() {
        ...
        let pair: MathPair = serde_json::from_str(&line)?;
        pairs.push(pair);
    }

    println!("📊 Dataset: {} math pairs", pairs.len());
    if !pairs.is_empty() {
        println!("   Example: '{}' → '{}'\n",
                 pairs[0].input_polish, pairs[0].target_polish());
    }

    // Determine max output size
    let max_target_len = pairs.iter()
        .map(|p| GraphemeGraph::from_text(&p.target_polish()).input_nodes.len())
        .max()
        .unwrap_or(10);
    ...
    for epoch in 0..EPOCHS {
        ...
        // Process all pairs
        for pair in &pairs {
            let input_graph = GraphemeGraph::from_text(&pair.input_polish);
            let target_graph = GraphemeGraph::from_text(&pair.target_polish());
