...
- `recall(query)` and `query(pattern)` require graph similarity/subgraph matching
- Exact subgraph isomorphism is **NP-complete**
- Graph isomorphism is **GI-complete** (unknown if P or NP)

**Mitigation Strategy (REQUIRED)**:
1. Use **approximate graph similarity** (not exact isomorphism):
   - Weisfeiler-Leman kernel for O(n·m·k) similarity
   - Feature-based hashing for O(n) approximate retrieval
2. **Index by structural features**: node count, edge count, degree histogram
3. **Bound retrieval scope**: limit to top-k candidates by hash similarity
4. **Never use exact canonical forms** for large graphs

**Complexity Bounds**:
- Retrieval: O(k · similarity_cost) where k = candidate limit
- Insertion: O(1) with hash indexing
- Query matching: O(n²) for bounded pattern size
