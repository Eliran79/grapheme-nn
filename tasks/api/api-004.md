...
### ⚠️ NP-Hard Complexity Warnings

**1. Analogical Reasoning (`analogize`) - GRAPH ISOMORPHISM**
- Finding structural mappings between graphs is **GI-complete** or **NP-complete**
- Worst case: O(n!) for exact isomorphism
- **Mitigation (REQUIRED)**:
  - Use feature-based matching (node types, degree sequences)
  - Limit search to top-k candidates by structural similarity
  - Use greedy assignment with Hungarian algorithm O(n³)
  - Accept approximate mappings (not optimal)

**2. Inductive Reasoning (`common_structure`) - MAXIMUM COMMON SUBGRAPH**
- Finding MCS across examples is **NP-hard**
- Worst case: O(2^(n+m)) for exact MCS
- **Mitigation (REQUIRED)**:
  - Limit to k examples where k ≤ 10
  - Use incremental intersection (pairwise MCS)
  - Accept approximate common structure
  - Bound graph sizes: n ≤ 1000 nodes

**3. Deductive Reasoning (`prove`) - POTENTIALLY UNDECIDABLE**
- Full first-order logic is **undecidable**
- Propositional logic is **NP-complete** (SAT)
- **Mitigation (REQUIRED)**:
  - Use restricted logic (Horn clauses, datalog)
  - Depth-bounded proof search (max 20 steps)
  - Timeout after T seconds

**Complexity Bounds (after mitigation)**:
- Analogy: O(n³) with Hungarian + O(k) candidates
- Induction: O(k · n²) for k examples
- Deduction: O(d · r) for depth d, rules r
